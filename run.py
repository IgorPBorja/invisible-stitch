...
def generate_scene(
    img: Image.Image, prompt: str, output_path: str = "./output.ply", mode: Literal["single", "stage", "360"] = "stage", seed: int = 0,
    ...
    # safe resize (preserves aspect ratio, while keeping every dimension <= max_resolution)
...
def main(
    image_path: str, prompt: str, output_path: str = "./output.ply", mode: Literal["single", "stage", "360"] = "stage", seed: int = 0,
    max_resolution: int = 1000
):
    img = Image.open(image_path).convert("RGB")
    generate_scene(img, prompt, output_path, mode, seed, max_resolution)
